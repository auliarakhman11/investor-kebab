...
class InvestorController extends Controller
{
    ...
    public function dtJurnal(Request $request)
    {
        ...
        $cabang = Jurnal::select('cabang_id', 'tgl')->where('tgl', '>=', $request->tgl1)->where('tgl', '<=', $request->tgl2)->where('void', 0)->where('cabang_id', '!=', 0)->groupBy('cabang_id')->groupBy('tgl')->get();
        $dat_penjualan = PenjualanKasir::select('cabang_id', 'tgl')->selectRaw("SUM(qty * harga_normal) as total_jual")->where('void', '!=', 2)->where('tgl', '>=', $request->tgl1)->where('tgl', '<=', $request->tgl2)->groupBy('cabang_id')->groupBy('tgl')->get();
        $dat_invoice = InvoiceKasir::select('cabang_id', 'tgl')->selectRaw("SUM(diskon) as jml_diskon")->where('void', '!=', 2)->where('tgl', '>=', $request->tgl1)->where('tgl', '<=', $request->tgl2)->groupBy('cabang_id')->groupBy('tgl')->get();
        $dat_persen_gaji_office = PenjualanGajiOffice::select('cabang_id', 'tgl')->selectRaw('SUM( (persen_gaji * jumlah) / 100) as persen_office ')->where('tgl', '>=', $request->tgl1)->where('tgl', '<=', $request->tgl2)->groupBy('cabang_id')->groupBy('tgl')->get();
        $dat_persen_gaji = PenjualanGaji::select('cabang_id', 'tgl', 'persen_gaji')->where('tgl', '>=', $request->tgl1)->where('tgl', '<=', $request->tgl2)->groupBy('cabang_id')->groupBy('tgl')->get();
        $dat_gapok_office = GapokOffice::select('cabang_id', 'tgl')->selectRaw('SUM(gapok) as ttl_gapok')->where('tgl', '>=', $request->tgl1)->where('tgl', '<=', $request->tgl2)->groupBy('cabang_id')->groupBy('tgl')->get();
        $dat_biaya_oprasional = Jurnal::select('cabang_id', 'tgl')->selectRaw('SUM(debit) as ttl_debit')->leftJoin('akun_pengeluaran', 'jurnal.akun_id', '=', 'akun_pengeluaran.id')->where('void', 0)->where('tgl', '>=', $request->tgl1)->where('tgl', '<=', $request->tgl2)->whereIn('jenis_akun_id', [5, 3])->groupBy('cabang_id')->groupBy('tgl')->get();
        $dat_barang_kebutuhan = Jurnal::select('cabang_id', 'tgl')->selectRaw("SUM(kredit) as ttl_kebutuhan")->where('tgl', '>=', $request->tgl1)->where('tgl', '<=', $request->tgl2)->where('void', 0)->where('akun_id', 14)->groupBy('cabang_id')->groupBy('tgl')->get();
        $dat_stok = Stok::select('cabang_id', 'tgl')->selectRaw("SUM(IF(jenis = 'Keluar',harga,0)) as harga_keluar, SUM(IF(jenis = 'Refund', harga,0)) as harga_refund")->where('stok.tgl', '>=', $request->tgl1)->where('stok.tgl', '<=', $request->tgl2)->whereIn('jenis', ['Keluar', 'Refund'])->groupBy('stok.cabang_id')->groupBy('stok.tgl')->get();
        ...
        $data_penjualan_investor = PenjualanInvestor::select('penjualan_investor.persen', 'penjualan_investor.tgl', 'penjualan_investor.cabang_id', 'penjualan_investor.investor_id')->where('tgl', '>=', $request->tgl1)->where('tgl', '<=', $request->tgl2)->get();
        $penjualan_investor = [];
        ...
        $data = [
            'dt_perhitungan' => $laba_investor,
            // 'ttl_laba' => $ttl_laba,
            'tgl1' => $request->tgl1,
            'tgl2' => $request->tgl2,
        ];
        return view('component.dt_investor', $data)->render();
    }
    ...
    public function dtPerOutlet(Request $request)
    {
        ...
        $dt_jurnal = ViewJurnal::selectRaw("SUM(laba_rugi) as sum_laba, SUM(ttl_stok) as sum_stok, SUM(ttl_varian) as sum_varian, SUM(ttl_barang_kebutuhan) as sum_kebutuhan, SUM(ttl_penjualan) as sum_penjualan, SUM(ttl_gapok_office + ttl_persen_crew + ttl_persen_office + ttl_gapok_crew) as sum_gaji, SUM(ttl_investor) as sum_investor, SUM(ttl_biaya_oprasional) as sum_biaya, dt_investor.persen, SUM(ttl_rusak) as sum_rusak, SUM(ttl_penjualan_cabang) as sum_penjualan_cabang, SUM(ttl_gapok_crew) as sum_gapok_crew, SUM(ttl_gapok_office) as sum_gapok_office, SUM(ttl_persen_crew) as sum_persen_crew, SUM(ttl_persen_office) as sum_persen_office")
            ...
            ->where('tgl', '>=', $tgl1)->where('tgl', '<=', $tgl2)->where('id', $cabang_id)->groupBy('cabang_id')->first();

        $sum_penjualan = $dt_jurnal->sum_penjualan;
        $sum_penjualan_antar_cabang = $dt_jurnal->sum_penjualan_cabang;

        $sum_bahan = $dt_jurnal->sum_stok;
        $sum_varian = $dt_jurnal->sum_varian;
        $sum_kebutuhan = $dt_jurnal->sum_kebutuhan;

        $sum_gapok_crew = $dt_jurnal->sum_gapok_crew;
        $sum_gapok_office = $dt_jurnal->sum_gapok_office;
        $sum_persen_crew = $dt_jurnal->sum_persen_crew;
        $sum_persen_office = $dt_jurnal->sum_persen_office;
