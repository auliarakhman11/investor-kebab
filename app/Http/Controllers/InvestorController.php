...
class InvestorController extends Controller
{
    ...
    public function dtJurnal(Request $request)
    {
        ...
        if ($dt_cabang_investor) {
            ...
            $dat_persen_gaji_office = PenjualanGajiOffice::select('cabang_id', 'tgl')->selectRaw('SUM( (persen_gaji * jumlah) / 100) as persen_office ')->where('tgl', '>=', $request->tgl1)->where('tgl', '<=', $request->tgl2)->whereIn('cabang_id', $dt_cabang)->where('void', 0)->groupBy('cabang_id')->groupBy('tgl')->get();
            $dat_persen_gaji = PenjualanGaji::select('cabang_id', 'tgl', 'persen_gaji')->where('tgl', '>=', $request->tgl1)->where('tgl', '<=', $request->tgl2)->whereIn('cabang_id', $dt_cabang)->where('void', 0)->groupBy('cabang_id')->groupBy('tgl')->get();
    ...
    public function dtPerOutlet(Request $request)
    {
        ...
        $dat_persen_gaji_office = PenjualanGajiOffice::select('cabang_id', 'tgl')->selectRaw('SUM( (persen_gaji * jumlah) / 100) as persen_office ')->where('tgl', '>=', $request->tgl1)->where('tgl', '<=', $request->tgl2)->where('cabang_id', $request->cabang_id)->where('void', 0)->groupBy('cabang_id')->groupBy('tgl')->get();
        $dat_persen_gaji = PenjualanGaji::select('cabang_id', 'tgl', 'persen_gaji')->where('tgl', '>=', $request->tgl1)->where('tgl', '<=', $request->tgl2)->where('cabang_id', $request->cabang_id)->where('void', 0)->groupBy('cabang_id')->groupBy('tgl')->get();
